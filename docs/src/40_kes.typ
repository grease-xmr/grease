...
#let algo(caption: none, title: none, list) = {
  figure(
    caption: caption,  kind: "algorithm",  supplement: [Algorithm], placement: auto,
    pseudocode-list(booktabs: true, title:title)[#list]
  )
}

= The Key Escrow Service (KES) Design

== Introduction

In short, the KES will hold an encrypted secret, #w0, from each of 2 parties. Either party can request their counterpart's secret if certain conditions are met.

Therefore the KES needs a keypair for each channel (#kg, #Pg) under which both parties can encrypt their secrets and send it to the KES for safe-keeping using `EncryptMessage` (@encM). If the conditions are satisfied, the KES can decrypt #w0 and send it to the claimant.

The KES only participates in the channel during the initialization, force-close and, dispute stages. It is not active in the update and co-operative close phases. The KES also has an optional cleanup step after a channel has closed. @kes-stages summarizes the KES' role throughout the channel lifecycle.
...
$EE_1$ is defined over a prime finite field $"GF"_1(p_1)$, with $p_1$ a large prime. Let $N_1$ be the size of the group generated by the Generator point, $G_1$ for group $GG_1$, such that $N.G equiv cal(O)$.

$EE_2$ is a SNARK-friendly curve and is similarly defined.

#H2F([...]) is a one-way function under the random oracle assumption that hashes the binary representation of its input to a scalar element in the group order. Domain separation and other hygiene considerations are assumed to be properly taken care of.

Similarly, #H2P([...]) is a one-way function under the random oracle assumption that hashes the binary representation of its input to a group element in $GG$.

The KES can utilize $EE_1$, $EE_2$, or both, depending on where it's deployed and which dispute resolution option (@forceClose) is selected. The VCOF utilizes a ZK-SNARK, and will therefore utilize $EE_2$.

In Grease, $EE_1$ is always Curve25519@curve25519, the curve behind Monero.
$EE_2$ can be any suitable curve. For the reference implementation of Grease, $EE_2$ is the BabyJubJub curve.
...
On creation, the KES creates a new, master keypair $(#kk, #Pk)$ on $EE_K$.
$#Pk$ is made available publicly.
...
#algo(
  ...
  [
    ...
    + $chs arrow.l H2F("ks, channel_id")$ on $EE_K$
...
Even though $hat(k)_a)$ must be unique for every channel opening, clients do not have to store a large database of keys (although the can). One could use a *master key*, $k_a$ and derive unique channel keys using a simple deterministic scheme,
...
- If #kg becomes compromised, the channel is compromised, but other current and future channels are still safe, since the attacker does not know #chs for the other channels.
- If #kk becomes compromised, channels are _still_ safe, as long as the #chs for any given channel is unknown. This is why is it critical to discard #chs after deriving the keys. While the damage would be contained, a compromised #kk is incredibly serious and would require the KES to start afresh with a new master keypair and inform all parties to close open channels immediately.
...
The Merchant (M) and Customer (C) partially sign a 2-of-2 multisig transaction with an adaptor signature.  Either party needs to know their counterpart's adapter signature offset, #w0 to complete the signature and spend the funds out of the multisig wallet, thus closing the channel.

The primary role of the KES during channel initialization is to store the encrypted #w0 values for each party.
The full procedure for calculating #w0 is described in @initProtocol.

It is important to note that the  adapter signature offsets, $w0^C$ and $w0^M$ and their corresponding points, $#PubWEd(0)^C$ and $#PubWEd(0)^M$ are elements of $EE_1$, the Ed25519 curve. For the KES to work with them, they need to be translated onto $EE_K$.

The offsets are chosen such that they are valid scalars on both curves, that is
$ 0 < w0 < min(N_1, N_2) $ // REVIEW - is this safe? Are we introducing too much bias here? If so, how else to to this?

The group element on $EE_1$ corresponding to #w0 is #PubWEd(0), while the corresponding point on $EE_K$ is denoted $PubWBjj(0)$. A summary of the points and their equivalents is given in @curveCorresp.

#note[
  It is possible that $EE_K$ is also Curve25519. This does not change the requirements or procedures. However, many things are much simpler. The DLEQ proofs are trivial, for instance.
]

=== What the parties send to the KES
Each party computes a discrete-log equivalence proof (DLEQ), $DleqP_0$ that proves that $PubWEd(0)$ and $PubWBjj(0)$ are generated from the same scalar, #w0.
...
The Merchant performs the analogous tasks and forwards both his and the customer's data to the KES. He also sends his DLEQ proof, $DleqP_0^M$ the the Customer, who validates it.
...
1.  Bob posts a "trigger transaction" to the KES smart contract on L2. This transaction includes his decryption key $k_(d i)^B$, the state index $i$, and the L2 signatures $sigma_i^('A), sigma_i^('B)$. This starts a timer.

2.  Alice has a time window to react. She has two options:
    -   *If state $i$ is the latest, correct state:* She cooperates by posting a "release transaction", revealing her decryption key $k_(d i)^A$. Bob can then use this key to recover Alice's signature and close the channel on L1. This option has the same outcome as the co-operative close but with the added overhead of involving the KES and paying L2 gas fees. It is significantly cheaper for Alice and Bob to co-operate.
...
3.  *Timeout:* If Alice does not respond within the time window, the KES contract automatically releases her *initial* decryption key $k_(d 0)^A$ on L2.

4.  *Resolution:*
    -   If Alice released $k_(d i)^A$, Bob can close the channel at state $i$.
    -   If the KES released $k_(d 0)^A$, Bob can use the `KeyUpdate` function to sequentially derive all subsequent keys $(k_(d 1)^A, ..., k_(d i)^A)$ up to the state he claimed. He can then recover Alice's signature for state $i$ and close the channel. This mechanism also acts as a punishment: if Alice was the malicious party trying to force an old state, Bob can now use her released initial key to derive the decryption key for any *later* state $j > i$ that is favorable to him, and close the channel at that state, stealing her funds.
...
Either party in the channel, the _claimant_, can trigger a force-close at any time. In short, the claimant informs the KES about the intent to force close, along with the current update count of the channel, $n$.

If the claim is valid, the KES starts the dispute window. If the counterparty, the _defendant_, has not presented a dispute proof before the window closes, the KES either
...
If a claimant wishes to initiate a force close they send a `ForceCloseRequest` message to the KES which triggers an `onForceCloseRequestEvent` event. The KES responds to the request event with the `HandleForceCloseRequest` procedure, outlined in @handleForceCloseRequest.
...
#algo(
  ...
  + *if* valid:
    ...
    + *if* `req.claimant != id.req.claimant` *return* Fail(Unauthorized)
    + *if* current time $>=$ pending.expiry
      ...
      + update `pending`, set `status` to `ForceClosed`, add `(Pc, Xc, n)` to `chi`
...
=== `ClaimChannelRequest` message <pendingChannelClose>
...
The KES responds to a `ClaimChannelRequest` message by triggering an `onClaimChannelRequestEvent` event.
It is handled by `handleClaimChannelRequest` as described in @handleClaimChannelRequest.
...
#note[In the "abandoned" window, that is, between the closing of the dispute window and the record deletion (@cleanup), the claimant can claim the channel using either type of message, but it is recommended to use `ClaimChannelRequest`.]
...
Under Option I, if there exists a prior channel state that is more disadvantageous to her than the claimed closing state, Alice is incentivized to publish a `ConsensusCloseRequest` (@consensusCloseRequest) which will close the channel in the state submitted by Bob in the `ForceCloseRequest`.

#note[It is strictly more expensive and time-consuming for both parties to close a channel under consensus rather than a co-operative close (@coopClose), with exactly the same result.]

When the KES receives a `ConsensusCloseRequest` message, it triggers an `onConsensusCloseRequestEvent` which leads to the `handleConsensusCloseRequest` methods carrying out @consensusClose.
...
The #link(label("forceCloseRequest"))[`ForceCloseRequest` message] contains a signature from both parties signing the update count. This is the same information that parties exchange on _every_ channel update. Therefore, it is now very straightforward for the KES to verify a dispute.
...
If all three checks pass, the KES decrypts $#w0^B$ with @decM, encrypts it to $P_A$ using @encM and sends the encrypted $w0^B$ to Alice.

=== Do Nothing

Alice can do nothing, in which case the dispute window will eventually close and Bob can claim the missing signature offset via @handleClaimChannelRequest.

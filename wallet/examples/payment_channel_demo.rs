...
async fn main() -> Result<(), WalletError> {
    ...
    println!("{:?}", get_signatureshare_scalar(&ss_a_encrypted));
...
fn make_adapted_shares(signature_shares: &SignatureShare<Ed25519>, secret: Scalar) -> SignatureShare<Ed25519> {
    //TODO: ensure this is a deep copy!
    let mut adapted_shares = signature_shares.clone();
    let mut buf = vec![];
    signature_shares.write(&mut buf).unwrap();
    let mut repr: [u8; 32] = [0u8; 32];
    repr.copy_from_slice(&buf[0..32]);
    let s = Scalar::from_repr(repr).unwrap();
    ...
    unsafe {
        update_signatureshare(&mut adapted_shares, s_adapted);
    }
    adapted_shares
}

fn adapt_shares(adapted_signature_shares: &SignatureShare<Ed25519>, secret: Scalar) -> SignatureShare<Ed25519> {
    //TODO: ensure this is a deep copy!
